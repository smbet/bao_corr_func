...
sbelem = subsamp(linestotal, 0.2)
good = np.where(sbelem == 1)[0]
lines = linestotal[good]

del(lines2)
del(linestotal) 

#random subsample:
RA_rdm = np.empty(len(lines))
DEC_rdm = np.empty(len(lines))

for ct,x in enumerate(lines):
    RA_rdm[ct] = (x.split(' ')[0])
    DEC_rdm[ct] = (x.split(' ')[1])

del(lines)

### got to this point, maybe just make a subsampled and full copy of everything random
...
#rdd for jackknife subsampler:
rdd_dat = np.array([RA_obs_filter,DEC_obs_filter,cosmo.comoving_radial_distance(Z_obs_filter)])
rdd_rdm = np.array([RA_rdm,DEC_rdm,cosmo.comoving_radial_distance(Z_rdm_obs)])

# xyz
d_pos = np.transpose(pos_dat)
r_pos = np.transpose(pos_rdm)

# RA/Dec/dist again
jack_positions = rdd_dat
subsampler = KMeansSubsampler('angular', positions=jack_positions, nsamples=njac, nside=512, random_state=42, position_type='rdd')
data_samples = subsampler.label(rdd_dat)
randoms_samples = subsampler.label(rdd_rdm)
...
result_mocks = TwoPointCorrelationFunction('smu', edges, data_positions1=d_pos, randoms_positions1=r_pos, data_samples1=data_samples, randoms_samples1=randoms_samples,
engine='corrfunc', nthreads=ncpu)
result_mocks.save('cor_func_good_'+subfrac+'.npy')

s_m, xiell_m, cov = project_to_multipoles(result_mocks, ells=ells, return_cov=True)
