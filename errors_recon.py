...
sbelem = subsamp(positions_rec_randoms, 0.2)
...
shifted_samples = subsampler.label(rdd_rec_pos)
shifted_randoms_samples = subsampler.label(rdd_rec_randoms)
...
### Compute un and re-constructed correlation functions:

result_mocks_rec = TwoPointCorrelationFunction('smu', edges, data_positions1=ds_pos, randoms_positions1=r_pos_rec, data_samples1=shifted_samples, randoms_samples1=shifted_randoms_samples,
engine='corrfunc', njac=njac, nthreads=ncpu)
result_mocks_rec.save('recon_cor_func_good_'+subfrac+'_smooth'+str(smoothrad)+'.npy')

s_m_rec, xiell_m_rec, covr = project_to_multipoles(result_mocks_rec, ells=ells, return_cov=True)
